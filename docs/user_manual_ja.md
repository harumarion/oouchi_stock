...
-## 在庫一覧画面
- 下部メニューの「在庫一覧」から移動するとカテゴリを SegmentedButton で切り替えられます。カテゴリを変更すると、そのカテゴリの在庫のみが一覧表示されます。
...
- 数値入力欄をタップすると画面下からスライドするキーパッドが表示されます。
  キーパッドで入力した値はすぐに反映されます。
...
- 数量や価格の欄をタップすると画面下から数値キーパッドが現れます。

-## セール情報編集画面
- セール情報詳細画面右上の鉛筆アイコンをタップすると開きます。
- 追加画面と同じ項目を編集できます。
- 数値入力欄をタップするとキーパッドが表示され、指で簡単に修正できます。
...
- 画面上部の検索バーから手動で商品名を追加できます。入力中は在庫商品の候補が表示されます。
- 買い物カードには現在の在庫数と予測される残り日数が表示されます。
- 各カード上部に購入予定日が大きく表示され、重要度に応じた色のチップが付きます。
