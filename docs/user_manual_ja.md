...
内部的には、ユースケースやリポジトリの初期化を共通の `DependencyFactory` で管理するようになり、画面ごとに安定した処理が行われます。
2024年版では `RepositoryFactory`・`UsecaseFactory`・`EntityFactory` を分離し、役割ごとに責務を明確化しました。依存注入の組み合わせは `DependencyFactory` が仲介し、テスト時にも同様の構成で扱えるようになっています。
## フォントサイズの統一
...
画面によってカード幅が異なるという指摘を受け、最新版では全てのカードが画面横幅いっぱいに表示されるよう統一しました。
...
- v1.5 でカテゴリと検索バーの余白を他画面と同じ配置に統一しました。
...
## セール情報管理画面
- セール情報管理画面ではカテゴリごとに登録した情報を一覧できます。
- カテゴリごとに登録したセール情報をカード形式で一覧表示します。商品名と品種はカード左側にまとめて表示され、右側に価格情報とメニューボタンが並びます。タップすると詳細画面が開きます。
 - 不要になった情報はカードを右にスワイプすると削除確認のポップアップが表示されます。
 - 編集は詳細画面の鉛筆アイコンから行います。
 - 各カード右上のメニューボタンからは「買い物リストへ追加」のみ実行できます。詳細表示はカードをタップ、削除はスワイプで行います。
...
- v1.6 でカテゴリと検索バーの余白を他画面と揃えました。
- v1.7 で検索バーとリストの間隔を統一し、左右の余白も揃えました。
- 期限切れから1日経過した情報は非表示になりますが、画面上の「期限切れも表示」スイッチをオンにすると過去の情報も確認できます。
- 画面右上のメニューから設定画面を開けます。
- v1.5 より、カテゴリ切り替え後に一覧が更新されない不具合を修正しました。
...
- 買い物カードには現在の在庫数と予測される残り日数が表示されます。
  - 在庫一覧やセール情報、買い物予報画面の各カードでは右上のメニューボタンから
    「買い物リストに追加」の操作を選択できます。
    2024年6月の更新でメニューボタンの位置を全画面で右端に統一しました。
...
- v1.5 でカテゴリと検索バーの余白を在庫一覧画面と揃えました。
