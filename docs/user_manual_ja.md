...
また、商品登録・編集フォームやカテゴリの色選択、検索バーなどは共通ウィジェットとして切り出されています。どの画面でも同じ操作感で利用できます。
...
-## 在庫一覧画面
- 下部メニューの「在庫一覧」から移動するとカテゴリを SegmentedButton で切り替えられます。
- 在庫カードをタップすると「在庫調整」「使用量入力」「購入量入力」「買い物リストへ追加」の操作をまとめたボトムシートが表示されます。長押しすると詳細画面へ移動します。カードを右にスワイプすると削除確認ダイアログが表示されます。
- 在庫カードを削除すると、その商品に紐づく履歴やセール情報、買い物リストの項目も同時に削除されます。
- 数量の変更は履歴として自動記録され、あとから確認できます。
- 検索バーと並び替えドロップダウンはスクロールに合わせて隠れる SliverAppBar にまとめられます。商品名だけでなくカテゴリ名や品種名でも検索できます。
...
- カテゴリごとに登録したセール情報をカード形式で一覧表示します。カード左上に「品種/商品名」を一行で表示し、期限や価格情報が続きます。価格は「通常価格 セール価格 差額」の順に1行で表示され、次の行に「単価」が続きます。カードをタップすると詳細画面が開きます。
...
- 「バックアップ」「復元」ボタンで端末への保存や復元を行えます。バックアップがない場合は「バックアップデータがありません」と表示されます。
  ボタン右側には実行日時が表示され、未実行の場合は「未バックアップ」や「未復元」と表示されます。
